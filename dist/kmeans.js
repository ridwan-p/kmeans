...
class Kmeans {
  constructor(vector, k) {
    if (k > vector.length) throw new Error('Required: # of points >= # of clusters');
    this.k = k;
  ...
  run() {
    ...
    vecArr = vecArr.map((x, i) => {
      const clusterSize = newCluster[i] ? newCluster[i].length : 0; // referensi https://www.baeldung.com/java-k-means-clustering-algorithm
      // when cluster size is empty , return centroid

      if (!clusterSize) return x; // return x.map(y => Math.floor(y / clusterSize))
